...
LRESULT CALLBACK DummyWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DRAWITEM: {
        ...
        if (p->CtlID == 1) {
            SetTextColor(p->hDC, RGB(140, 35, 35));
        } else {
...
void ExecuteQueue(HWND hwnd, BOOL isSingleShot) {
    ...
    for (int i = 0; i < queueCount; i++) {
        if (strlen(queue[i].customExe) == 0) needsJson = TRUE;
    }
    if (needsJson) {
        int waitLoops = 0;
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
    case WM_CREATE: {
        CreateWindowA("STATIC", "Game Name:", WS_CHILD | WS_VISIBLE, 27, 25, 200, 20, hwnd, NULL, NULL, NULL);
        hGameName = CreateWindowA("EDIT", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 27, 45, 240, 22, hwnd, (HMENU)13, NULL, NULL);
        CreateWindowA("STATIC", "Custom EXE Path (Optional):", WS_CHILD | WS_VISIBLE, 27, 75, 200, 20, hwnd, NULL, NULL, NULL);
        hCustomExe = CreateWindowA("EDIT", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL, 27, 95, 240, 22, hwnd, (HMENU)13, NULL, NULL);
        CreateWindowA("STATIC", "Time (s):", WS_CHILD | WS_VISIBLE, 27, 125, 200, 20, hwnd, NULL, NULL, NULL);
        hTime = CreateWindowA("EDIT", "910", WS_CHILD | WS_VISIBLE | WS_BORDER, 27, 145, 100, 22, hwnd, (HMENU)14, NULL, NULL);
    ...
    case WM_COMMAND: {
        int id = LOWORD(wParam);
        int code = HIWORD(wParam);
        if (id == 2) { ExecuteQueue(hwnd, TRUE); }
            ...
            if (editingQueueIndex != -1) {
                strcpy(queue[editingQueueIndex].gameName, gInput);
                strcpy(queue[editingQueueIndex].customExe, cExe);
                queue[editingQueueIndex].timeSec = t;
        ...
        else if ((id == 12 || id == 13 || id == 14) && code == EN_CHANGE) {
            if (!ignoreEditChange && editingQueueIndex != -1) {
                char btnText[32]; GetWindowTextA(hBtnAddQueue, btnText, 32);
                if (strcmp(btnText, "Save") != 0) {
